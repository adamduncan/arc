...
const Frtabs = function ({
	...
	function _showTab (target, giveFocus = true) {
		...
		let siblingTabs = _q(tablistSelector + ' a[tabindex="0"]', thisContainer);
		let siblingTabpanels = _q(tabpanelSelector + ':not([aria-hidden="true"])', thisContainer);
		let tabPanelId = target.getAttribute('aria-controls')
		...
		doc.getElementById(tabPanelId).removeAttribute('aria-hidden');
		if (giveFocus) target.focus();

		if (onShowTab && typeof onShowTab === 'function' && giveFocus) onShowTab();
	}
